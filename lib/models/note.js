    ...
    async.series({
      parseNoteIdByAlias: function (_callback) {
        ...
        Note.findOne({
          ...
          // By default docs in the config.docsPath are created / updated as locked
          // so nobody can edit them but everyone (even without login) can read them.
          // In the specificPublicDocsPermissions dict you can specify a key with the
          // name of the file (without the .md suffix) and set the individual permission
          // as the value.
          let permissionForDoc = 'locked'
          if (noteId in config.specificPublicDocsPermissions) {
            permissionForDoc = config.specificPublicDocsPermissions.noteId
          }
          if (note) {
